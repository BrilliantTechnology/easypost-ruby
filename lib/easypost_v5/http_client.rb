...
class EasyPostV5::HttpClient
  ...
  def request(
    ...
    serialized_body = JSON.dump(EasyPostV5::InternalUtilities.objects_to_ids(body)) if body
    open_timeout = @config[:open_timeout]
    read_timeout = @config[:read_timeout]
    request_timestamp = Time.now
    request_uuid = SecureRandom.uuid

    if EasyPostV5::Hooks.any_subscribers?(:request)
      request_context = EasyPostV5::Hooks::RequestContext.new(
        method: method,
        path: uri.to_s,
        headers: headers,
        request_body: body,
      ...
      EasyPostV5::Hooks.notify(:request, request_context)
    end
    ...
    if EasyPostV5::Hooks.any_subscribers?(:response)
      response_context = {
        http_status: nil,
        method: method,
        path: uri.to_s,
      ...
      EasyPostV5::Hooks.notify(:response, EasyPostV5::Hooks::ResponseContext.new(**response_context))
    end
  ...
  def default_request_execute(method, uri, headers, open_timeout, read_timeout, body = nil)
    ...
    begin
      ...
      raise EasyPostV5::Errors::TimeoutError.new(e.message)
    rescue OpenSSL::SSL::SSLError => e
      # Raise an SSL error if the request fails due to an SSL error.
      raise EasyPostV5::Errors::SslError.new(e.message)
      ...
      raise EasyPostV5::Errors::UnknownApiError.new(e.message)
    end
  end
end
