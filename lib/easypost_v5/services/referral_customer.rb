...
class EasyPostV5::Services::ReferralCustomer < EasyPostV5::Services::Service
  MODEL_CLASS = EasyPostV5::Models::User
  ...
  def add_credit_card(referral_api_key, number, expiration_month, expiration_year, cvc, priority = 'primary')
    ...
    begin
      ...
      raise EasyPostV5::Errors::ExternalApiError.new(EasyPostV5::Constants::STRIPE_CARD_CREATE_FAILED)
    end
  ...
  # Retrieve EasyPostV5's Stripe public API key.
  def retrieve_easypost_stripe_api_key
    response = @client.make_request(:get, 'partners/stripe_public_key', EasyPostV5::Models::EasyPostObject, nil, 'beta')
    response['public_key']
  end
  ...
  def create_stripe_token(number, expiration_month, expiration_year,
    ...
    form_encoded_params = EasyPostV5::InternalUtilities.form_encode_params(credit_card_hash)
  ...
  # Submit Stripe credit card token to EasyPostV5.
  def create_easypost_credit_card(referral_api_key, stripe_object_id, priority = 'primary')
    ...
    referral_client = EasyPostV5::Client.new(api_key: referral_api_key)
    referral_client.make_request(:post, 'credit_cards', EasyPostV5::Models::EasyPostObject, wrapped_params, 'beta')
  end
end
